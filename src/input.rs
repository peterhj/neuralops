...
pub struct ParallelVarInputOperator<S, IoBuf: ?Sized> {
  cfg:      VarInputOperatorConfig,
  node:     OperatorNode,
  out:      CommonOutput,
  rng:      Xorshiftplus128Rng,
...
impl<S, IoBuf: ?Sized> ParallelVarInputOperator<S, IoBuf> {
  pub fn new(cfg: VarInputOperatorConfig, cap: OpCapability) -> Rc<RefCell<ParallelVarInputOperator<S, IoBuf>>> {
    let batch_sz = cfg.batch_sz;
    let mut tmp_buf = Vec::with_capacity(batch_sz * cfg.max_stride);
    tmp_buf.resize(batch_sz * cfg.max_stride, 0.0);
...
impl<S, IoBuf: ?Sized> Operator for ParallelVarInputOperator<S, IoBuf> {
  fn _next(&self) -> u64 {
    self.node._next()
  }
}

impl<S, IoBuf: ?Sized> CommonOperator for ParallelVarInputOperator<S, IoBuf> {
  fn _output(&self, arm: usize) -> CommonOutput {
    assert_eq!(0, arm);
    self.out.clone()
  }
...
impl<IoBuf: ?Sized> DiffOperator<SampleItem, IoBuf> for ParallelVarInputOperator<SampleItem, IoBuf> {
  ...
  fn _load_batch(&mut self, samples: &[SampleItem]) {
    ...
    let mut out_buf = self.out.buf.borrow_mut();
    out_buf.reshape_mut(self.cfg.batch_sz * self.cfg.max_stride).parallel_set_constant(0.0);
    self.in_dims.clear();
    for (idx, sample) in samples.iter().enumerate() {
      if sample.kvs.contains::<SampleSharedExtractInputKey<[f32]>>() {
        let data = sample.kvs.get::<SampleSharedExtractInputKey<[f32]>>().unwrap();
        data.parallel_extract_input(&mut (&mut *out_buf)[idx * self.cfg.max_stride .. (idx+1) * self.cfg.max_stride]).unwrap();
      } else {
        panic!();
      }
      if sample.kvs.contains::<SampleInputShapeKey<(usize, usize, usize)>>() {
  ...
  fn _forward(&mut self, phase: OpPhase) {
      ...
      match preproc {
        &VarInputPreproc::Scale{scale} => {
          for idx in 0 .. batch_size {
            let dim = self.tmp_dims[idx];
            let mut out = &mut (&mut *out_buf)[idx * self.cfg.max_stride .. (idx+1) * self.cfg.max_stride];
            out.reshape_mut(dim.flat_len()).parallel_scale(scale);
          }
        }
        &VarInputPreproc::DivScale{scale} => {
          for idx in 0 .. batch_size {
            let dim = self.tmp_dims[idx];
            let mut out = &mut (&mut *out_buf)[idx * self.cfg.max_stride .. (idx+1) * self.cfg.max_stride];
            out.reshape_mut(dim.flat_len()).parallel_div_scalar(scale);
          }
        }
        /*&VarInputPreproc::ChannelShift{ref shift} => {
          for idx in 0 .. batch_size {
          ...
          if phases.contains(&phase) {
            for idx in 0 .. batch_size {
              ...
              let out_len = out_dim.flat_len();
              {
                let out = &(&*out_buf)[idx * self.cfg.max_stride .. (idx+1) * self.cfg.max_stride];
                let mut tmp = &mut self.tmp_buf[idx * out_len .. (idx+1) * out_len];
                unsafe { neuralops_omp_interpolate2d_catmullrom(
                    in_dim.0, in_dim.1, in_dim.2,
                    out_dim.0, out_dim.1,
                    out.as_ptr(),
                    tmp.as_mut_ptr(),
              ...
              let mut out = &mut (&mut *out_buf)[idx * self.cfg.max_stride .. (idx+1) * self.cfg.max_stride];
              //out[ .. out_len].copy_from_slice(&tmp);
              out.reshape_mut(out_len).parallel_copy(tmp.reshape(out_len));
              self.tmp_dims[idx] = out_dim;
            }
          }
        }
          ...
          if phases.contains(&phase) {
            for idx in 0 .. batch_size {
              ...
              let offset_y = self.rng.gen_range(0, in_dim.1 + 2 * pad_h - crop_h + 1) as isize - pad_h as isize;
              {
                let out = &(&*out_buf)[idx * self.cfg.max_stride .. (idx+1) * self.cfg.max_stride];
                let mut tmp = &mut self.tmp_buf[idx * out_len .. (idx+1) * out_len];
                unsafe { neuralops_omp_image_crop(
                    in_dim.0, in_dim.1, in_dim.2,
                    out_dim.0, out_dim.1,
                    offset_x, offset_y,
                    out.as_ptr(),
                ...
                1 => {
                  {
                    let out = &(&*out_buf)[idx * self.cfg.max_stride .. (idx+1) * self.cfg.max_stride];
                    let mut tmp = &mut self.tmp_buf[idx * out_len .. (idx+1) * out_len];
                    unsafe { neuralops_omp_image_flip(
                        out_dim.0, out_dim.1, out_dim.2,
                        out.as_ptr(),
                        tmp.as_mut_ptr(),
                    ) };
    ...
    for idx in 0 .. batch_size {
      ...
      let mut tmp = &mut self.tmp_buf[idx * out_len .. (idx+1) * out_len];
      //tmp.copy_from_slice(&out[ .. out_len]);
      tmp.reshape_mut(out_len).parallel_copy(out.reshape(out_len));
    }
    //out_buf[ .. batch_size * out_len].copy_from_slice(&self.tmp_buf[ .. batch_size * out_len]);
    out_buf.reshape_mut(batch_size * out_len).parallel_copy(self.tmp_buf.reshape(batch_size * out_len));
    //println!("DEBUG: varinput: output: {:?}", &out_buf[ .. out_len]);
