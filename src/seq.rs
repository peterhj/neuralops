...
pub enum SeqOperatorConfig {
  ...
  LstSqRegressLoss(RegressLossConfig),
  NormLstSqRegressLoss(NormLstSqRegressLossConfig),
}
...
pub struct RegressSeqOperator<T, S> {
  input_op:     Box<DiffOperatorInput<T, S, Output=CommonOperatorOutput<f32>, Rng=Xorshiftplus128Rng>>,
  loss_op:      Box<DiffOperatorInput<T, S, Output=CommonOperatorOutput<f32>, Rng=Xorshiftplus128Rng>>,
  inner_ops:    Vec<Box<DiffOperator<T, Output=CommonOperatorOutput<f32>, Rng=Xorshiftplus128Rng>>>,
}

impl<S> RegressSeqOperator<f32, S> where S: SampleDatum<[f32]> + SampleLabel + SampleLossWeight<RegressLoss> {
  pub fn new(cfgs: Vec<SeqOperatorConfig>, cap: OpCapability) -> RegressSeqOperator<f32, S> {
    let res = CommonResources::new();
    let num_ops = cfgs.len();
    let input_op: Box<DiffOperatorInput<f32, S, Output=CommonOperatorOutput<f32>, Rng=Xorshiftplus128Rng>> = match &cfgs[0] { //Box<DiffOperator<f32, Output=_, Rng=_> = match &cfgs[0] {
      &SeqOperatorConfig::SimpleInput(ref cfg) => {
    ...
    let loss_op: Box<DiffOperatorInput<f32, S, Output=CommonOperatorOutput<f32>, Rng=Xorshiftplus128Rng>> = match cfgs[num_ops-1] {
      SeqOperatorConfig::LstSqRegressLoss(cfg) => {
        let prev_op: &DiffOperator<f32, Output=CommonOperatorOutput<f32>, Rng=Xorshiftplus128Rng> = match inner_ops.len() {
          0 => &*input_op.as_op(),
          _ => &*inner_ops[inner_ops.len()-1] as &DiffOperator<f32, Output=CommonOperatorOutput<f32>, Rng=Xorshiftplus128Rng>,
        };
        let op = LstSqRegressLossOperator::new(cfg, cap, prev_op, 0, res.clone());
        Box::new(op)
      }
      SeqOperatorConfig::NormLstSqRegressLoss(cfg) => {
        let prev_op: &DiffOperator<f32, Output=CommonOperatorOutput<f32>, Rng=Xorshiftplus128Rng> = match inner_ops.len() {
          0 => &*input_op.as_op(),
          _ => &*inner_ops[inner_ops.len()-1] as &DiffOperator<f32, Output=CommonOperatorOutput<f32>, Rng=Xorshiftplus128Rng>,
        };
        let op = NormLstSqRegressLossOperator::new(cfg, cap, prev_op, 0, res.clone());
        Box::new(op)
      }
      _ => unreachable!(),
    };
...
impl<T, S> DiffOperatorInput<T, S> for RegressSeqOperator<T, S> {
  fn load_data(&mut self, samples: &[S]) {
    self.input_op.load_data(samples);
    self.loss_op.load_data(samples);
  }
...
impl<T, S> DiffOperator<T> for RegressSeqOperator<T, S> {
  type Output = CommonOperatorOutput<f32>;
  type Rng = Xorshiftplus128Rng;
...
impl<S> DiffOperatorOutput<f32, RwSlice<f32>> for RegressSeqOperator<f32, S> {
  fn get_output(&mut self) -> RwSlice<f32> {
    //self.loss_out.out_buf.as_slice()
    self.loss_op._output(0).out_buf.as_slice()
  }
